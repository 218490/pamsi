...
/*!
 *Wyszukuje podany element w drzewie
 *
 *\param[in] element- poszukiwana wartosc
 *
 *\retval zwraca adres do poszukiwanego elementu w drzewa w przypadku 
 *znalezienia elementu lub NULL w przypadku przeciwnym
 */
Pojemnik* Drzewo::Wyszukaj(int element){
  Pojemnik *pom;
  pom=head;
  while(pom!=NULL){
    if(pom->WezWartosc()>element)
      pom=pom->Lewy();
    else if(pom->WezWartosc()<element)
      pom=pom->Prawy();
    else
      return pom;
  }
